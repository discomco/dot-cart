...
{
    public readonly ICouchViewDefinitionB View;
    ...
    public CouchQuery(ICouchViewDefinitionB view)
    {
        View = view;
    }
    ...
    {
        var bulk = new BulkKeys(keys.ToArray());
        Data(Document.WriteJson(bulk));
        return this;
    }
    ...
    public GenericViewResult GetResult()
    {
        try
        {
            return GetResult<GenericViewResult>();
        }
    ...
    public ViewResultStream<T> StreamResult<T>() where T : ICanJson, new()
    {
        try
        {
            return new ViewResultStream<T>(Request().Stream());
        }
        catch (WebException e)
        {
            throw CouchException.Create("Query failed", e);
