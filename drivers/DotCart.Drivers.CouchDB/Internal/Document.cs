...
/// </summary>
public class Document : IReconcilingDocument
{
    ...
    public Document(IDictionary<string, JToken> doc)
    ...
    /// </summary>
    protected Document SourceData { get; private set; }

    public virtual ReconcileStrategy ReconcileBy { get; set; } = ReconcileStrategy.None;
    ...
    protected Document AutoClone()
    {
        var doc = GetType().GetConstructor(Type.EmptyTypes).Invoke(new object[0]) as Document;
        var properties =
            GetType().GetProperties(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
        var fields = GetType().GetFields(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
        foreach (var field in fields)
    ...
    protected virtual Document Clone()
    {
        var doc = (Document)GetType().GetConstructor(Type.EmptyTypes).Invoke(new object[0]);
        doc.Rev = Rev;
        doc.Id = Id;
        return doc;
    }
    ...
    public virtual void ReadJson(JObject obj)
    {
        ...
        {
            SourceData = (Document)GetType().GetConstructor(Type.EmptyTypes).Invoke(new object[0]);
