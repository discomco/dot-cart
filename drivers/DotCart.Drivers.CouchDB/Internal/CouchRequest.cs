...
public class CouchRequest : ICouchRequest
{
    ...
    {
        var requestUri = new UriBuilder("http",
            _server.Settings.Host,
            _server.Settings.Port,
            (_db != null ? _db.Name + "/" : "") + _path,
            _query).Uri;
        if (WebRequest.Create(requestUri) is not HttpWebRequest request)
        ...
        if (!string.IsNullOrEmpty(_server.Settings.EncodedCredentials))
            request.Headers.Add("Authorization", _server.Settings.EncodedCredentials);
