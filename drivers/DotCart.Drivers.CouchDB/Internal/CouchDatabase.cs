...
///     This is the main API to work with CouchDB.
/// </summary>
public class CouchDatabase : ICouchDatabase
{
    public readonly IList<DesignDocument>
        DesignDocuments = new List<DesignDocument>();
    ...
    public string Name
    {
        get
        {
            if (Server == null)
                return _name;
            return Server.Settings.DatabasePrefix + _name;
        }
        set => _name = value;
    }
    ...
    public DesignDocument NewDesignDocument(string aName)
    {
        var newDoc = new DesignDocument(aName, this);
    ...
    /// </summary>
    public ICouchViewDefinitionB NewTempView(string designDoc, string viewName, string mapText)
    {
    ...
    public IEnumerable<JsonDocument> GetAllDocuments()
    {
    ...
    public IEnumerable<Document> GetAllDocumentsWithoutContent()
    {
        QueryAllDocuments().GetResult().ValueDocuments<Document>();

        var list = new List<Document>();
        var json = RequestAllDocuments().Parse();
        foreach (JObject row in json["rows"])
            list.Add(new Document(row["id"].ToString(), row["value"]["rev"].ToString()));
        return list;
    }
    ...
    public void Delete()
    {
        if (Exists()) Server.DeleteDatabase(Name);
    }

    /// <summary>
    ///     Write a document given as plain JSON and a document id. A document may already exist in db and will then be
    ///     overwritten.
    /// </summary>
    ...
    {
        return WriteDocument(new JsonDocument(json, documentId));
    }
    ...
    {
        return ((DocumentWrapper<T>)SaveDocument(new DocumentWrapper<T>(document))).Instance;
    }
    ...
    public ICouchDocument SaveDocument(ICouchDocument document)
    {
        ...
        try
        {
            savedDoc = document.Id == null ? CreateDocument(document) : WriteDocument(document);
        }
    ...
    public ICouchDocument WriteDocument(ICouchDocument document, bool batch)
    {
        ...
        var result =
            Request(document.Id).Query(batch ? "?batch=ok" : null).Data(Document.WriteJson(document)).Put()
                .Check("Failed to write document").Result();
    ...
    /// <remarks>This relies on the document to already have an id.</remarks>
    public ICouchDocument WriteAttachment(ICouchDocument document, string attachmentName, string attachmentData,
        string mimeType)
    {
    ...
    /// <returns>The document.</returns>
    /// <remarks>This relies on the document to already have an id.</remarks>
    public ICouchDocument WriteAttachment(ICouchDocument document, string attachmentName, byte[] attachmentData,
        string mimeType)
    {
        if (document.Id == null)
            throw CouchException.Create(
        ...
        var result =
            Request(document.Id + "/" + attachmentName).Query("?rev=" + document.Rev).Data(attachmentData)
                .MimeType(mimeType).Put().Check("Failed to write attachment")
                .Result();
    ...
    /// <returns>The document.</returns>
    /// <remarks>This relies on the document to already have an id.</remarks>
    public ICouchDocument WriteAttachment(ICouchDocument document, string attachmentName, Stream attachmentData,
        string mimeType)
    {
        if (document.Id == null)
            throw CouchException.Create(
    ...
    {
        if (HasDocumentChanged(document)) ReadDocument(document);
    }
    ...
    {
        return (JsonDocument)CreateDocument(new JsonDocument(json));
    }

    /// <summary>
    ///     Create a given ICouchDocument in CouchDB. Uses POST and CouchDB will allocate a new id and overwrite any existing
    ///     id.
    /// </summary>
    ...
    public ICouchDocument CreateDocument(ICouchDocument document)
    {
        try
        {
            var result = Request().Data(Document.WriteJson(document)).PostJson()
                .Check("Failed to create document").Result();
    ...
    {
        SaveDocuments(documents.Select(doc => new DocumentWrapper<T>(doc)),
            allOrNothing);
    }
    ...
    {
        var bulk = new BulkDocuments(documents);
        try
        {
            var result = Request("_bulk_docs")
                .Data(Document.WriteJson(bulk))
    ...
    public void SaveArbitraryDocuments<T>(IEnumerable<T> documents, int chunkCount,
        IEnumerable<ICouchViewDefinitionT> views, bool allOrNothing)
    {
        SaveDocuments(
            documents.Select(doc => new DocumentWrapper<T>(doc)),
            chunkCount,
            views,
            allOrNothing);
    }
    ...
    public void SaveDocuments(IEnumerable<ICouchDocument> documents, int chunkCount,
        IEnumerable<ICouchViewDefinitionT> views, bool allOrNothing)
    {
    ...
    public void TouchViews(IEnumerable<ICouchViewDefinitionT> views)
    {
    ...
    public IEnumerable<JsonDocument> GetDocuments(IEnumerable<string> documentIds)
    {
        return GetDocuments<JsonDocument>(documentIds);
    }

    public IEnumerable<T> GetDocuments<T>(IEnumerable<string> documentIds) where T : ICouchDocument, new()
    {
        var bulk = new BulkKeys(documentIds);
        return QueryAllDocuments().Data(Document.WriteJson(bulk)).IncludeDocuments().GetResult()
            .Documents<T>();
    }
    ...
    {
        var doc = new DocumentWrapper<T>(ctor)
        {
            Id = documentId
        };
        try
    ...
    public JsonDocument GetDocument(string documentId)
    {
        try
        {
            try
            {
                return new JsonDocument(Request(documentId).Parse());
            }
            catch (WebException e)
            {
    ...
    {
        return Query(new ViewDefinitionT(viewName, NewDesignDocument(designName)));
    }

    public CouchQuery Query(ICouchViewDefinitionB view)
    {
    ...
    {
        var result = Request(document.Id + "/" + attachmentName).Query("?rev=" + document.Rev).Delete()
            .Check("Failed to delete attachment").Result();
        document.Id = result["id"].Value<string>(); // Not really neeed
        document.Rev = result["rev"].Value<string>();
        return document;
    }
    ...
    {
        var docs = QueryAllDocuments().StartKey(startKey).EndKey(endKey).GetResult().RowDocuments()
            .Cast<ICouchDocument>();
    ...
    {
        DeleteDocuments(new BulkDeleteDocuments(documents));
    }
    ...
    public void DeleteDocuments(ICanJson bulk)
    {
        try
        {
            var result = Request("_bulk_docs").Data(Document.WriteJson(bulk)).PostJson().Parse<JArray>();
