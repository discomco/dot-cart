...
public class RedisItem<T> : RedisObject
{
    ...
    public new RedisItem<T> WithTx(RedisTransactionDb redisDb)
    {
        return base.WithTx(redisDb) as RedisItem<T>;
    }

    public new RedisItem<T> WithBatch(BatchRedisDb batchRedisDb)
    {
        return base.WithBatch(batchRedisDb) as RedisItem<T>;
    }
    ...
    /// <returns></returns>
    public static Task<bool> SetMultiple(IList<KeyValuePair<RedisItem<T>, T>> keyValuePairs,
