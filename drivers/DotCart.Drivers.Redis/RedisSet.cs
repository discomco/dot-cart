...
public class RedisSet<T> : RedisObject, IAsyncEnumerable<T>
{
    ...
    public new RedisSet<T> WithTx(RedisTransactionDb redisDb)
    {
        return base.WithTx(redisDb) as RedisSet<T>;
    }

    public new RedisSet<T> WithBatch(BatchRedisDb batchRedisDb)
    {
        return base.WithBatch(batchRedisDb) as RedisSet<T>;
    }
