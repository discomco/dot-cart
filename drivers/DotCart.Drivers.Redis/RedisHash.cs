...
public class RedisDtoHash<TDto> : RedisValueHash
{
    ...
    public new RedisDtoHash<TDto> WithTx(RedisTransactionDb redisDb)
    {
        return base.WithTx(redisDb) as RedisDtoHash<TDto>;
    }

    public new RedisDtoHash<TDto> WithBatch(BatchRedisDb batchRedisDb)
    {
        return base.WithBatch(batchRedisDb) as RedisDtoHash<TDto>;
    }
}
...
public class RedisHash<TKey, TValue> : RedisObject, IAsyncEnumerable<KeyValuePair<TKey, TValue>>
{
    ...
    public new RedisHash<TKey, TValue> WithTx(RedisTransactionDb redisDb)
    {
        return base.WithTx(redisDb) as RedisHash<TKey, TValue>;
    }

    public new RedisHash<TKey, TValue> WithBatch(BatchRedisDb batchRedisDb)
    {
        return base.WithBatch(batchRedisDb) as RedisHash<TKey, TValue>;
    }
    ...
    public async IAsyncEnumerator<KeyValuePair<TKey, TValue>> GetAsyncEnumerator()
    {
        await foreach (var entry in Executor.HashScanAsync(KeyName))
            yield return new KeyValuePair<TKey, TValue>(ToElement<TKey>(entry.Name),
                ToElement<TValue>(entry.Value));
    }
}