...
public class RedisList<T> : RedisObject, IAsyncEnumerable<T>
{
    ...
    public new RedisList<T> WithTx(RedisTransactionDb redisDb)
    {
        return base.WithTx(redisDb) as RedisList<T>;
    }

    public new RedisList<T> WithBatch(BatchRedisDb batchRedisDb)
    {
        return base.WithBatch(batchRedisDb) as RedisList<T>;
    }
