...
public class RedisSortedSet<T> : RedisObject, IAsyncEnumerable<T>
{
    ...
    public new RedisSortedSet<T> WithTx(RedisTransactionDb redisDb)
    {
        return base.WithTx(redisDb) as RedisSortedSet<T>;
    }

    public new RedisSortedSet<T> WithBatch(BatchRedisDb batchRedisDb)
    {
        return base.WithBatch(batchRedisDb) as RedisSortedSet<T>;
    }
    ...
    /// <param name="take"></param>
    /// <returns></returns>
    public Task<IList<T>> RangeByScore(double start = double.NegativeInfinity,
    ...
    /// <param name="exclude"></param>
    /// <returns></returns>
    public Task<long> RemoveRangeByScore(double start = double.NegativeInfinity,
    ...
    {
        var getKeys = getKeyNamePattern == null ? null : getKeyNamePattern.Select(s => (RedisValue)s).ToArray();
        return Executor.SortAndStoreAsync(destination.KeyName, KeyName, skip, take, order, sortType,
